...
from utils import (
    generate_graph_historical_and_forecasted,
    plot_hourly_barchart,
    plot_gauge,
)
...
st.sidebar.markdown(
    "Welcome to SPARC California! This app allows you to see the forecasted carbon emissions of common daily activities."
)
st.sidebar.markdown(
    "At any moment, the electricity that we use from the grid comes from a mix of many sources, from natural gas to solar to nuclear depending on resource availability, weather, time, day, and season. The more nonrenewable resources are used to provide energy to the grid, the more CO2 emissions are produced."
)
st.sidebar.markdown(
    "If we can predict how much CO2 we are producing at a given time, we can adjust our behavior to reduce emissions!"
)
st.sidebar.markdown("")
st.sidebar.markdown(
    "*Kun Guo, Nina Prakash, Griffin Schillinger Tarpenning | Stanford University | CS 329s*"
)
...
hour = st.selectbox(
    "Select a time in the next 24 hours",
    ["12:00am"]
    + [str(h) + ":00am" for h in range(1, 12)]
    + ["12:00pm"]
    + [str(h) + ":00pm" for h in range(1, 12)],
    index=13,
)

duration = st.selectbox(
    "Select a duration", ["1 hour"] + [str(i) + " hours" for i in range(2, 25)]
)

choice = st.radio(
    "Pick one",
    [
        "Use default energy consumption of {}: {} kWh".format(
            activity_present_tense[activity], ACTIVITY_USAGE_KWH[activity]
        ),
        "Enter my own",
    ],
)
...
if choice == "Enter my own":
    energy_cons = st.text_input(
        f"Energy consumption of {activity_present_tense[activity]} (kW):",
        ACTIVITY_USAGE_KWH[activity],
    )
    try:
...
if clicked_generate:
    ...
    with st.empty():
        ...
        if results and fig2 and fig3:
            co2, co2_perc, recommended_time = compute_co2(
                results, energy_cons, hour, duration
            )
    ...
    # Write output
    st.subheader(
        f"To {activity[0].lower() + activity[1:]} at {hour} for {duration}, you will produce: "
    )
    ...
    if fig2 and fig3:
        ...
        with st.expander("Click to see recommendation"):
            st.subheader(
                f'Based on the forecast results, it is recommended that you begin {activity_present_tense[activity]} at {recommended_time.strftime("%I:00%p")} on {recommended_time.strftime("%b %d")}'
            )
