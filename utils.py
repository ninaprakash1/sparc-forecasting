...
def plot_hourly_barchart(results):
    ...
    results_perc = (
        results.drop(columns=["time", "total"])
        .div(results.drop(columns=["time", "total"]).sum(axis=1), axis=0)
        .round(3)
        * 100
    )
    ...
    fig = go.Figure()
    fig.add_bar(
        name="Solar", x=time_idx, y=results_perc["solar"], marker_color=colors["solar"]
    )
    fig.add_bar(
        name="Wind", x=time_idx, y=results_perc["wind"], marker_color=colors["wind"]
    )
    fig.add_bar(
        name="Hydro", x=time_idx, y=results_perc["hydro"], marker_color=colors["hydro"]
    )
    fig.add_bar(
        name="Other Renewables",
        x=time_idx,
        y=results_perc["renewable_other"],
        marker_color=colors["renewable_other"],
    )
    fig.add_bar(
        name="Battery",
        x=time_idx,
        y=results_perc["battery"],
        marker_color=colors["battery"],
    )
    fig.add_bar(
        name="Fossil Fuels",
        x=time_idx,
        y=results_perc["fossil_fuel"],
        marker_color=colors["fossil_fuel"],
    )
    fig.add_bar(
        name="Other", x=time_idx, y=results_perc["other"], marker_color=colors["other"]
    )
    fig.update_layout(
        barmode="relative",
...
def generate_graph_historical_and_forecasted():
    ...
    data2 = data2.head(
        len(data2) - int(datetime.now(timezone("US/Pacific")).minute / 5)
    )
    ...
    if (
        "result" not in str(res.text)
        or type(json.loads(res.text)["result"]["battery"]) == int
    ):
    ...
    results = {
        grouped_source: list(res[grouped_source].values())
        for grouped_source in res.keys()
    }
    hours_from_pred = []
    for i in range(1, 25):
    ...
    data2_temp = data2_temp.rename(
        columns={
            ...
            "Wind": "wind",
            "Hydro": "hydro",
            "Other Renewables": "renewable_other",
            "Batteries": "battery",
            "Other": "other",
        }
    )
    ...
    fig = go.Figure(
        data=[
            go.Scatter(
                name="Solar",
                x=hist_and_future_concat["time"],
                y=hist_and_future_concat["solar"],
            ...
            go.Scatter(
                name="Hydro",
                x=hist_and_future_concat["time"],
                y=hist_and_future_concat["hydro"],
            ...
            go.Scatter(
                name="Other Renewables",
                x=hist_and_future_concat["time"],
                y=hist_and_future_concat["renewable_other"],
                marker_color=colors["renewable_other"],
                mode="lines",
            ),
            go.Scatter(
                name="Battery",
                x=hist_and_future_concat["time"],
                y=hist_and_future_concat["battery"],
                marker_color=colors["battery"],
                mode="lines",
            ),
            go.Scatter(
                name="Fossil Fuels",
                x=hist_and_future_concat["time"],
                y=hist_and_future_concat["fossil_fuel"],
                marker_color=colors["fossil_fuel"],
                mode="lines",
            ),
            go.Scatter(
                name="Other",
                x=hist_and_future_concat["time"],
                y=hist_and_future_concat["other"],
            ...
            go.Scatter(
                ...
                y=[0, hist_and_future_concat.drop(columns=["time"]).values.max()],
                marker_color="red",
                mode="lines",
                line=dict(dash="dash"),
            ),
